...
var Utils = {
	Safe: {
		dataCheck: function dataCheck(dataSource) {
			...
			if (data[0].length <= maxIndex)
				throw 'Chart input data is length(' +
					data[0].length +
					') is less than required data index(' +
					maxIndex +
					')'
		}
	},
	...
	Math: {
		...
		iterOffsetN: function iterOffsetN(data, index, n, callback) {
			...
			else if (target > data.length - 1) target = data.length - 1
     ...
     */
		...
		linearActual2Display: function linearActual2Display(val, coord) {
			return (
				(val - coord.actual[0]) * (coord.display[1] - coord.display[0]) / (coord.actual[1] - coord.actual[0]) +
				coord.display[0]
			)
		},
		linearDisplay2Actual: function linearDisplay2Actual(pos, coord) {
			return (
				(pos - coord.display[0]) * (coord.actual[1] - coord.actual[0]) / (coord.display[1] - coord.display[0]) +
				coord.actual[0]
			)
		},
		seekNeatPoints: function seekNeatPoints(range, count) {
			...
			multiples.forEach(function(multiple) {
				...
				points.push(
					[ Number((newRange[0] - interval).toFixed(fixPrecision)) ].concat(newRange, [
						Number((newRange[newRange.length - 1] + interval).toFixed(fixPrecision))
					])
				)
			})
	...
	Grid: {
		...
		calcGridLines: function calcGridLines(actual, lineCount, baseValue) {
			...
			if (baseValue !== undefined && typeof baseValue === 'number') {
				...
				lines = [].concat(
					_toConsumableArray(hm.slice(0, -2)),
					_toConsumableArray(
						hm.slice(0, -1).reverse().map(function(h) {
...
var calcOHLC = function calcOHLC(data, coord, seriesConf) {
	...
	data.forEach(function(item, index) {
		...
		var direction =
			c === o && index > 0
				? data[index - 1][seriesConf.closeIndex] < item[seriesConf.closeIndex] ? 'up' : 'down'
				: c < o ? 'up' : 'down'
		res[direction].push({
			x: ~~item.x,
			low: l,
		...
		Draw.FillnStroke(
			...
			function(ctx) {
				candles[direction].forEach(function(candle) {
					ctx.rect(
						~~(candle.x - half) + 0.5,
						~~Math.min(candle.open, candle.close) + 0.5,
						half * 2,
...
function columnPainter(ctx, data, coord, seriesConf, viewport) {
	...
	data.forEach(function(item, index) {
		...
		var baseVal =
			seriesConf.baseVal !== undefined
				? seriesConf.baseVal
				: seriesConf.baseIndex !== undefined ? item[seriesConf.baseIndex] : null
	...
	for (var direction in columns) {
		Draw.FillnStroke(
					...
					if (seriesConf.mode === 'bidirection') {
						ctx.rect(
							...
							~~(
								...
								Utils.Coord.linearActual2Display(seriesConf.baseVal, coordY)
							) + 0.02
						) // + 0.02 is for IE fix
					} else {
						ctx.rect(
			...
			seriesConf.style.block[direction],
			seriesConf.style.border[direction]
		)
	}
}
					...
					if (seriesConf.mode === 'bidirection') {
						ctx.rect(
							...
							~~Utils.Coord.linearActual2Display(item[seriesConf.valIndex], coord.y) -
								~~Utils.Coord.linearActual2Display(baseVal, coord.y) +
								0.02
						)
					} else {
						ctx.rect(
							...
							Draw.Text(
								...
								function(ctx) {
									coord.horizLines.forEach(function(y, index) {
										var val = Utils.Math.valueFormat(
											y.actual,
											style.valueFormatter,
											style.valuePrecision
										)
								...
								Draw.Text(
									...
									function(ctx) {
										...
										coord.verticalLines.forEach(function(x) {
											ctx.fillText(
												dateFormatter(x.actual, style.dateFormat),
								...
								seriesInfo.timeRanges.forEach(function(range, index) {
									...
									Draw.Text(
										...
										function(ctx) {
											...
											ctx.textBaseline = style.axis.label[direction].textBaseline
											ctx.fillText(
												dateFormatter(range[0], style.dateFormat),
												displayRange[0] + 5,
												viewport[direction] + style.axis.label[direction].offset.y
											)
						...
						if (mainSeries) {
							...
							Draw.Fill(
								...
								function(ctx) {
									ctx.rect(
										x,
										_y - style.tip.currPrice.labelHeight / 2,
										width,
							...
							Draw.Text(
								...
								function(ctx) {
									ctx.fillText(
										Utils.Math.valueFormat(value, style.valueFormatter, style.valuePrecision),
...
function textLabelPainter(_ref) {
	...
	realOrigin.x =
		realOrigin.x < 0 ? 0 : realOrigin.x + labelWidth > bound.xMax ? bound.xMax - labelWidth : realOrigin.x
	Utils.Draw.Fill(
		ctx,
		function(ctx) {
...
var events = {
	crosshair: function crosshair(chart, e, linked) {
		...
		Utils.Draw.Stroke(
			...
			function(ctx) {
				...
				if (!linked) {
					chart.eventInfo.yPos =
						chart.style.crosshair.snapToData && snapPointIndex && chart.eventInfo.selectedItem
							? ~~Utils.Coord.linearActual2Display(
									chart.eventInfo.selectedItem[snapPointIndex],
									chart.dataProvider.coord.y
								)
							: e.localY
	...
	axisLabel: function axisLabel(chart, e, linked) {
		...
		textLabelPainter({
			...
			origin: {
				...
				y:
					chart.style.crosshair.axisLabel.posOffset.xAxisLabel.y +
					(chart.style.crosshair.axisLabel.xAxisLabelPos === 'bottom'
						? chart.viewport.bottom
						: chart.viewport.top - chart.style.crosshair.axisLabel.height)
			},
		...
		if (!snapPointIndex) return
		var horizPos =
			chart.style.crosshair.snapToData && chart.eventInfo.selectedItem
				? ~~Utils.Coord.linearActual2Display(
						chart.eventInfo.selectedItem[snapPointIndex],
						chart.dataProvider.coord.y
					)
		...
		if (!linked) {
			hoverValue = Utils.Math.valueFormat(
				Utils.Coord.linearDisplay2Actual(horizPos, chart.dataProvider.coord.y),
				chart.style.valueFormatter,
				chart.style.valuePrecision
			)
		} else {
			hoverValue = 0
		}
		...
		textLabelPainter({
			...
			origin: {
				x:
					chart.style.crosshair.axisLabel.posOffset.yAxisLabel.x +
					(chart.style.crosshair.axisLabel.yAxisLabelPos === 'right' ? chart.viewport.right : 0),
	...
	selectDot: function selectDot(chart, e, linked) {
		...
		chart.style.crosshair.selectedPoint.color.forEach(function(color, index) {
			Utils.Draw.Fill(
				...
				function(ctx) {
					ctx.arc(
						chart.eventInfo.selectedItem.x + 0.5,
						Utils.Coord.linearActual2Display(
							chart.eventInfo.selectedItem[snapPointIndex],
							chart.dataProvider.coord.y
						) - 1.5,
	...
	toolTip: function toolTip(chart, e, linked) {
		...
		if (
			!chart.style.crosshair.toolTip ||
			Object.prototype.toString.call(chart.style.crosshair.toolTip) !== '[object Function]'
		)
			return
	...
	panMove: function panMove(chart, e, linked) {
		...
		if (
			(e.deltaX < 0 && newOffset < chart.viewport.right - chart.viewport.left - chart.viewport.barWidth * 5) ||
	...
	pinchMove: function pinchMove(chart, e, linked) {
		...
		if (
			chart.eventInfo.pinchStart.barWidth + oldBarWidth * (zoomScale / scaleDivision) > 4 &&
			chart.eventInfo.pinchStart.barWidth + oldBarWidth * (zoomScale / scaleDivision) < 64
		) {
			chart.viewport.barWidth = chart.eventInfo.pinchStart.barWidth + oldBarWidth * (zoomScale / scaleDivision)
			chart.viewport.offset =
				chart.eventInfo.pinchStart.offset - offsetIndex * oldBarWidth * (zoomScale / scaleDivision)
		}
	...
	mouseWheel: function mouseWheel(chart, e, linked) {
		...
		if (
			chart.viewport.barWidth + oldViewport * (zoomScale / scaleDivision) > 4 &&
			chart.viewport.barWidth + oldViewport * (zoomScale / scaleDivision) < 64
		) {
		...
		for (var l = filteredData.length - 1; l > 0; l--) {
			if ((xpos - filteredData[l]) * (xpos - filteredData[l - 1]) < 0) {
				if (Math.abs(xpos - filteredData[l]) < Math.abs(xpos - filteredData[l - 1])) {
					return [
						chart.dataSource[l + chart.dataProvider.filteredData.leftOffset],
						l + chart.dataProvider.filteredData.leftOffset
					]
				} else {
					return [
						chart.dataSource[l - 1 + chart.dataProvider.filteredData.leftOffset],
						l - 1 + chart.dataProvider.filteredData.leftOffset
					]
				}
			}
		}
	...
	unscalable: function unscalable(chart, xpos) {
		...
		for (var l = filteredData[rangeIndex].length - 1; l > 0; l--) {
			if ((xpos - filteredData[rangeIndex][l].x) * (xpos - filteredData[rangeIndex][l - 1].x) < 0) {
				if (
					Math.abs(xpos - filteredData[rangeIndex][l].x) < Math.abs(xpos - filteredData[rangeIndex][l - 1].x)
				) {
			...
			},
			{
				...
				value: function genVerticalLines() {
					...
					if (this._chartType === 'unscalable') {
						...
						for (
							var l = this._filteredData.data.length - 1;
							l >= 0;
							l -= Math.round(style.grid.span.x / viewport.barWidth)
						) {
							if (
								this._filteredData.data[l].x > viewport.left &&
...
_defineProperty(_title$title$type$req, 'title', 'lavania config'),
_defineProperty(_title$title$type$req, 'type', 'object'),
_defineProperty(_title$title$type$req, 'required', [ 'type' ]),
_defineProperty(_title$title$type$req, 'properties', {
	type: {
		title:
			'chart type, use "scalable" to specify a chart that can be zoomed or paned, if scalable is defined, series.timeRanges will be ignored. use "unscalable" to specify a chart can not change viewport',
	...
	valueFormatter: {
		description:
			'if valuePrecision is not good enough for need, you can use valueFormatter provide a function to gennerate the priceText from value'
	},
	dateFormat: {
	...
	crosshair: {
		...
		properties: {
			snapToData: {
				title:
					'whether crosshair snap to the value or align with mouse position, when set to ture, crosshair will snap to the mainSeries(set in seriesInfo.mainSeriesIndex, fallback to 0)',
				type: 'boolean'
			},
			// toolTip: {
			...
			snapProperty: {
				title:
					'when snapToData is ture, specify a property which crosshair will snapto, if this prop is not defined, will try data[valIndex], if valIndex is not defined, will use data[closeIndex]',
				// type: 'string',
			...
			_defineProperty(_dash, 'title', 'refer to mdn: CanvasRenderingContext2D.setLineDash'),
			_defineProperty(_dash, 'type', 'array'),
			_dash),
			lineWidth: {
			...
			axisLabel: {
				...
				_defineProperty(_properties, 'posOffset', {
					...
					properties: {
						yAxisLabel: {
							title:
								'vertical position offset for the corsshair y-axis label, which can used for adjust the position',
							type: 'object',
							properties: {
								x: {
									title: 'horizone offset',
						...
						xAxisLabel: {
							title:
								'position offset for the corsshair x-axis label, which can used for adjust the position',
							type: 'object',
							properties: {
								x: {
									title: 'horizone offset',
	...
	seriesInfo: {
		...
		properties: {
			...
			mainSeriesIndex: {
				title:
					'specify the index of the main series, main series is used for crosshair to snap to, default to 0',
				type: 'number'
			},
			series: {
				title: 'series config',
